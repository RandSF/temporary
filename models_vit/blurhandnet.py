...
from losses import CoordLoss, ParamLoss, CoordLossOrderInvariant, DiversityPromotingLoss, VAELoss
# from models_pose.modules.ktformer import KTFormer
from models_vit. modules.transformer import Transformer
from models_vit.modules.regressor import Regressor
from models_vit.modules.resnetbackbone import ResNetBackbone
from models_vit.modules.layer_utils import init_weights
...
class BlurHandNet(nn.Module):
    ...
    def forward(self, inputs, targets, meta_info, mode):
        ...
        # feature enhancing via Transformer
        feat_joint, mu, logstd = self.transformer(feat_blur) # [B, K, T, E], [B, T, E], [B, T, E]
        ...
        if mode == 'train':
            ...
            feat_joint_all = feat_joint.transpose(0, 2)
            ...
            with torch.no_grad():
                ...
                idx = torch.argsort(loss_cam.mean([-1, -2]), dim=0, descending=False).unsqueeze(0)   # [1, K, B]

            feat_joint_sort = torch.gather(feat_joint_all, dim=1, index=idx[..., None].expand_as(feat_joint_all))   # [T, K, B, E]
            # below are [T, KS, B, ...]
            joint_proj = torch.gather(joint_proj_all, dim=1, index=idx[..., None, None].expand_as(joint_proj_all))[:, :KS]
            joint_cam = torch.gather(joint_cam_all, dim=1, index=idx[..., None, None].expand_as(joint_cam_all))[:, :KS]
            mano_pose = torch.gather(mano_pose_all, dim=1, index=idx[..., None].expand_as(mano_pose_all))[:, :KS]
            mano_shape = torch.gather(mano_shape_all, dim=1, index=idx[..., None].expand_as(mano_shape_all))[:, :KS]
            ...
            # KLD
            loss['VAE'] = self.opt_loss['lambda_vae'] * self.vae_loss(mu, logstd)
