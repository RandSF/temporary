...
# from model_current.modules.unfolder import Unfolder
from models_try.modules.resnetbackbone import ResNetBackbone
from models_try.modules.regressor import Regressor
from models_try.modules.transformer import Transformer
from models_try.modules.infonce import InfoNCE
from models_try.modules.layer_utils import init_weights
...
class BlurHandNet(nn.Module):
    def __init__(self, opt, weight_init=True):
        ...
        self.info_nce = InfoNCE(**opt['task_parameters'], **opt_net['infonce'])
        self.regressor = Regressor(**opt['task_parameters'], **opt_net['regressor'])  # Regressor
        self.trainable_modules = [self.backbone, self.transformer, 
                                  self.info_nce, 
                                  self.regressor]
        ...
        if weight_init:
            ...
            self.info_nce.apply(init_weights), 
    ...
    def forward(self, inputs, targets, meta_info, mode):
        ...
        # src = (feat_blur.flatten(-2)    # [B, C', H*W]
        #                 .transpose(1, 2)# [B, H*W, C']
        #        )
        src = feat_blur # [B, C', H, W]

        feat_spat = self.transformer(src)   # [B, K, T, E]
        
        E = feat_spat.shape[-1]

        # mano params prediction
        feat_mano = feat_spat
        mano_shape_flatten, mano_pose_flatten, camera_flatten = self.regressor(feat_blur, feat_mano)
        cam_tran_flatten = self.get_camera_trans(camera_flatten)    # [B*K*T, 3]
        ...
        joint_proj_flatten, joint_cam_flatten, mesh_cam_flatten = \
            self.get_coord(mano_pose_flatten, mano_shape_flatten, cam_tran_flatten)    # [B*K*T, J, 2], [B*K*T, J, 3], [B*K*T, V, 3]
        ...
        if mode == 'train':
            ...
            joint_proj_all = joint_proj_flatten.reshape(B, K, T, J, -1).transpose(0, 2)             # [T, K, B, J, 2]
            joint_cam_all = joint_cam_flatten.reshape(B, K, T, J, -1).transpose(0, 2)               # [T, K, B, J, 3]
            mesh_cam_all = mesh_cam_flatten.reshape(B, K, T, mano.vertex_num, -1).transpose(0, 2)   # [T, K, B, V, 3]
            mano_pose_all = mano_pose_flatten.reshape(B, K, T, -1).transpose(0, 2)                  # [T, K, B, \theta]
            mano_shape_all = mano_shape_flatten.reshape(B, K, T, -1).transpose(0, 2)                # [T, K, B, \beta]
            ...
            mesh_cam_all = mesh_cam_flatten.reshape(B, K, T, mano.vertex_num, -1).transpose(0, 1)   # [K, B, T, V, 3]
    ...
    def get_coord(self, pose, shape, cam_trans):
        batch_size = pose.shape[0]
        output = self.mano_layer_right(global_orient=pose[:, :3], hand_pose=pose[:, 3:], betas=shape)
