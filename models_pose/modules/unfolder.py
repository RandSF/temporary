...
from models_pose.modules.layer_utils import make_conv_layers, make_deconv_layers
...
class Unfolder(nn.Module):
    ...
    def forward(self, feat_blur, feat_pyramid):
        ...
        return torch.stack([feat_joint_e1, feat_joint_md, feat_joint_e2], dim=1), \
            torch.stack([joint_img_e1, joint_img_md, joint_img_e2], dim=1)
...
class Decoder(nn.Module):
    ...
    def forward(self, feat_blur, feat_pyramid):        
        ...
        x = self._upsample(feat_blur, feat_pyramid['stride16'])
        y = self.skip1(feat_pyramid['stride16'])
        x = torch.cat((x, y), 1)
        x = self.conv1(x)

        # x = self.deconv2(x)
        x = self._upsample(x, feat_pyramid['stride8'])
