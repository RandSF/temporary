...
class Unfolder(nn.Module):
    def __init__(self, num_joints, output_hm_shape, **kwargs):
        super().__init__()
    ...
    def forward(self, feat_blur, feat_pyramid):
        ...
        joint_img_e1 = soft_argmax_2d(pred_heatmap_e1.reshape(-1, self.num_joints, self.output_hm_shape[0], self.output_hm_shape[1]))
        joint_img_md = soft_argmax_2d(pred_heatmap_md.reshape(-1, self.num_joints, self.output_hm_shape[0], self.output_hm_shape[1]))
        joint_img_e2 = soft_argmax_2d(pred_heatmap_e2.reshape(-1, self.num_joints, self.output_hm_shape[0], self.output_hm_shape[1]))
        ...
        return torch.stack([feat_joint_e1, feat_joint_md, feat_joint_e2], 1), torch.stack([joint_img_e1, joint_img_md, joint_img_e2], 1)


class Decoder(nn.Module):
    def __init__(self, num_joints, output_hm_shape, in_chans=2048, out_chans=512, **kwargs):
        super().__init__()
        self.num_joints = num_joints
        self.output_hm_shape = output_hm_shape 
    ...
    def forward(self, feat_blur, feat_pyramid):        
        ...
        heatmap = self.conv_proj(x).reshape(-1, self.num_joints, self.output_hm_shape[0], self.output_hm_shape[1])
